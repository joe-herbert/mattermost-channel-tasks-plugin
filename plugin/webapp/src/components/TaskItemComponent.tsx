...
export const TaskItemComponent: React.FC<TaskItemComponentProps> = ({task, channelMembers, onToggle, onDelete, onToggleAssignee, onUpdateText, onDragStart, onDragEnd, onDragOverTask, onDragLeaveTask, onDropOnTask, isDraggingItem, isDropTarget, dropPosition, theme}) => {
    ...
    const handleAssignClick = (e: React.MouseEvent) => {
        ...
        console.log('Clicked assign: ', showAssigneePopup);
        setShowMenu(false);
    };
    ...
    const hasAssignees = assignedMembers.length > 0;
                ...
                <div style={{display: 'flex', alignItems: 'center', gap: "4px", position: "relative"}} data-task-background="true">
                            ...
                            <div style={{display: 'inline-block'}}>
                                ...
                                {isEditingDeadline && (
                                    <div style={{marginTop: '4px', marginLeft: '12px', display: 'flex', alignItems: 'center', gap: '4px'}}>
                                        ...
                                        {editDeadline && (
                                            <button
                                                onMouseDown={handleClearDeadline}
                                                style={{padding: '4px 8px', fontSize: '14px', color: errorTextColor, backgroundColor: 'transparent', border: 'none', cursor: 'pointer'}}
                    ...
                    {hasValidDeadline && !isEditingDeadline && (
                        <div
                            ...
                            style={{
                                ...
                                marginTop: '2px', fontWeight: deadlineColor ? 500 : 400,
                                cursor: 'pointer', padding: '2px 4px', borderRadius: '3px', display: 'inline-block', transition: 'background-color 0.2s'
                            }}
                            onMouseEnter={(e) => {
                    ...
                    <div style={{position: 'relative', marginRight: isHovered || showMenu || showAssigneePopup ? "20px" : "0px", transition: "margin-right 0.3s ease"}} ref={popupRef}>
                            ...
                            >
                                <div style={{display: 'flex', alignItems: 'center'}}>
                                    {assignedMembers.slice(0, 3).map((member, index) => (
                                        <img
                                            ...
                                            style={{
                                                ...
                                                position: 'relative', zIndex: 3 - index
                                            }}
                                    ...
                                    {assignedMembers.length > 3 && (
                                        <div style={{
                                            width: '28px', height: '28px', borderRadius: '50%',
                                            backgroundColor: dragHandleColor, color: centerChannelBg,
                                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            fontSize: '11px', fontWeight: 'bold',
                                            border: `2px solid ${centerChannelBg}`,
                                            marginLeft: '-10px', position: 'relative', zIndex: 0
                                        }}>
                                            +{assignedMembers.length - 3}
                                        </div>
                                    )}
                        ...
                        {showAssigneePopup && (
                            <div style={{
                                ...
                                backgroundColor: centerChannelBg, border: `1px solid ${popupBorder}`,
                                borderRadius: '4px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)',
                                zIndex: 1000, minWidth: '200px', maxHeight: '300px', overflowY: 'auto'
                            }}>
                                <div style={{padding: '8px 12px', borderBottom: `1px solid ${borderColor}`, fontSize: '12px', fontWeight: 600, color: dragHandleColor}}>
                                    Assign to
                                </div>
                                {channelMembers.map(member => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            style={{
                                                padding: '8px 12px', display: 'flex', alignItems: 'center', cursor: 'pointer',
                                                backgroundColor: isAssigned ? selectedBg : 'transparent',
                                                borderLeft: isAssigned ? `3px solid ${buttonBg}` : '3px solid transparent'
                                            }}
                                            onMouseEnter={(e) => {
                                                if (!isAssigned) e.currentTarget.style.backgroundColor = hoverBg;
                                            }}
                                            onMouseLeave={(e) => {
                    ...
                    <div style={{position: 'absolute', right: '0px', top: '50%', transform: 'translateY(-50%)', zIndex: 1000}}>
                        <button
                            ...
                            style={{
                                padding: '4px 6px', fontSize: '16px', color: dragHandleColor,
                                background: 'none', border: 'none', cursor: 'pointer', flexShrink: 0,
                                display: isHovered || showMenu || showAssigneePopup ? "block" : "none", transition: 'opacity 0.2s',
                                borderRadius: '3px'
                            }}
                            onMouseEnter={(e) => {
                                e.currentTarget.style.backgroundColor = hoverBg;
                        ...
                        {showMenu && (
                            ...
                            >
                                ...
                                >
                                    <i className="icon icon-calendar-outline" style={{fontSize: '16px', marginRight: '8px', color: dragHandleColor}}/>
                                    {hasValidDeadline ? 'Edit Deadline' : 'Set Deadline'}
                                </div>
                                <div
                                ...
                                >
                                    <i className="icon icon-account-outline" style={{fontSize: '16px', marginRight: '8px', color: dragHandleColor}}/>
                                    Assign
                                </div>
                                ...
                                >
                                    <i className="icon icon-trash-can-outline" style={{fontSize: '16px', marginRight: '8px'}}/>
                                    Delete Task
                                </div>
                            </div>
                        )}
